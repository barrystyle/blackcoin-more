...
bool CheckStakeKernelHash(const CBlockIndex* pindexPrev, unsigned int nBits, uint32_t blockFromTime, CAmount prevoutValue, const COutPoint& prevout, unsigned int nTimeTx, bool fPrintProofOfStake)
{
    ...
    if (UintToArith256(hashProofOfStake) < (UintToArith256(bestSeen.proofHash))) {
        bestSeen.proofHash = hashProofOfStake;
        bestSeen.timeSeen = GetAdjustedTime();
        LogPrintf("best proofhash seen %s\n", bestSeen.proofHash.ToString());
    }
