...
void CWallet::ReacceptWalletTransactions()
{
    ...
    for (std::pair<const uint256, CWalletTx>& item : mapWallet) {
        ...
        // Peercoin/Blackcoin: abandon orphaned transactions
...
bool CWalletTx::SubmitMemoryPoolAndRelay(std::string& err_string, bool relay)
{
    ...
    // Don't try to submit coinbase and coinstake transactions. These would fail anyway but would
    // cause log spam.
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        for (const auto& coin : selected_coins) {
            txNew.vin.push_back(CTxIn(coin.outpoint, CScript(), (CTxIn::SEQUENCE_FINAL - 1)));
        }
    ...
    WalletLogPrintf("Fee Calculation: Fee:%d Bytes:%u Needed:%d\n",
